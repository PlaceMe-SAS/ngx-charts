...
exports.ColorHelper = undefined;

var _d3Array = require('d3-array');

var _d3Scale = require('d3-scale');

var _colorSets = require('../utils/color-sets');

var ColorHelper = /** @class */function () {
    function ColorHelper(scheme, type, domain, customColors) {
        if (typeof scheme === 'string') {
            scheme = _colorSets.colorSets.find(function (cs) {
    ...
    ColorHelper.prototype.generateColorScheme = function (scheme, type, domain) {
        ...
        var colorScale;
        if (type === 'quantile') {
            colorScale = (0, _d3Scale.scaleQuantile)().range(scheme.domain).domain(domain);
        } else if (type === 'ordinal') {
            colorScale = (0, _d3Scale.scaleOrdinal)().range(scheme.domain).domain(domain);
        } else if (type === 'linear') {
            ...
            var points = (0, _d3Array.range)(0, 1, 1.0 / colorDomain.length);
            colorScale = (0, _d3Scale.scaleLinear)().domain(points).range(colorDomain);
        }
        return colorScale;
    };
    ColorHelper.prototype.getColor = function (value) {
        if (this.scaleType === 'linear') {
            var valueScale = (0, _d3Scale.scaleLinear)().domain(this.domain).range([0, 1]);
    ...
    ColorHelper.prototype.getLinearGradientStops = function (value, start) {
        ...
        var valueScale = (0, _d3Scale.scaleLinear)().domain(this.domain).range([0, 1]);
        var colorValueScale = (0, _d3Scale.scaleBand)().domain(this.colorDomain).range([0, 1]);
